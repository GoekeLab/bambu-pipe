class Validation {
    ...
    static def validateParams(params, workflow) {
        ...
        // Required inputs
        if (!params.input)
            throw new Exception("params.input is not set — please provide a path to a CSV samplesheet")
        if (!params.genome)
            throw new Exception("params.genome is not set — please provide a path to the reference genome FASTA file")
        if (!params.annotation)
            throw new Exception("params.annotation is not set — please provide a path to the reference annotation GTF file")
        ...
        if (!(params.bam_only instanceof Boolean))
            throw new Exception("Invalid params.bam_only '${params.bam_only}' — must be true or false")
